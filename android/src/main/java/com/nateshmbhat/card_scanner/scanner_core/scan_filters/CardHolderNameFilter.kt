...
class CardHolderNameFilter(
    ...
    private val cardHolderRegex: Regex = Regex(CardScannerRegexps.CARD_HOLDER_NAME, RegexOption.MULTILINE)
    ...
    override fun filter(): CardHolderNameScanResult? {
        ...
        val minTextBlockIndexToSearchName = max(
            cardNumberScanResult.textBlockIndex -
                    (if (scannerOptions.possibleCardHolderNamePositions.contains(CardHolderNameScanPositions.ABOVE_CARD_NUMBER.value)) 1 else 0),
            0
        )
        val maxTextBlockIndexToSearchName =
            min(
                cardNumberScanResult.textBlockIndex +
                        (if (scannerOptions.possibleCardHolderNamePositions.contains((CardHolderNameScanPositions.BELOW_CARD_NUMBER.value))) maxBlocksBelowCardNumberToSearchForName else 0),
