...
abstract class CardScannerRegexps {
    companion object {
        const val CARD_NUMBER_REGEX = "^(\\s*\\d\\s*){16}\$"
        const val EXPIRY_DATE_REGEX = "(0[1-9]|1[0-2])/([0-9]{2})"
        const val CARD_HOLDER_NAME =
            "^ *(([A-Z.]+ {0,2}){1,8}) *\$" // A line containing name has : minimum 1 word and maximum 8 words
    }
}
